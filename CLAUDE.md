...
- **Client grouping:** `CLIENT_GROUPS` mapping in `src/app/page.tsx` maps `client_name` values to display groups (e.g., `flooret-commercial` → "Flooret"). Must be updated when new clients are onboarded. Unmapped names pass through as-is.
- **User management:** Cognito SDK operations in `src/lib/aws/cognito.ts`. Admin-only API routes at `/api/users`. No multi-tenant check (global admin resource). Self-protection: API prevents admins from deleting/disabling themselves or removing themselves from the admin group. N+1 group fetching acceptable for <50 users. Username must not be email format (Cognito pool uses email as alias).
